	...
	// returns whether or not a number is prime
			public static boolean isPrime (int num) {
				...
				boolean prime = true;
					if (num > 1) {
						for (int i = 2; i < num; i++) {
								divisible = Quadratic.isDivisibleBy(num, i);
								if (divisible == true) { 
									prime = false;
									}	
							}
			...
			if (num2 < 0) {
				throw new IllegalArgumentException ("Can't determine gcf is num is negative, retuurns the wrong greatest");
					...
					for (int i = Quadratic.min(num1, num2); i > 0; i--) { // use the minimum to find gcf, not max
						if (Quadratic.max(num1, num2) % i == 0 && Quadratic.min(num1,num2) % i == 0) { 
...
//returns square root of a specific number
	...
	double quad = Quadratic.discriminant(a, b, c); //Set quad equal to calculate the discriminant of a, b, c
	String ans = ""; //makes ans a string
	if (quad == 0) { // if discriminant = 0, it will return 1 root
		double root = Quadratic.round2(-b/(2*a)); // equals 1 root, rounded to 2 decimal places
		ans = root + ""; // quotes create a string
	}
	if (quad > 0) {
		double firstRoot = Quadratic.round2((-b + Quadratic.sqrt(quad)))/(2*a); // quadratic formula for the plus
		double secondRoot = Quadratic.round2((-b - Quadratic.sqrt(quad)))/(2*a); // quadratic formula for the minus
	...
	// This method will describe a quadratic equation
	public static String quadrDescriber (double a, double b, double c) {
		System.out.println("Description of the graph of:" + " y = " + a + "x^2" + " + " + b + "x" + " + " + c);
	if (a < 0) {
	...
	System.out.println("x-intercept(s): " + Quadratic.quadForm(a, b, c));
